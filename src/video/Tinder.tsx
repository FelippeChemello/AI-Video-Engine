...
export const Tinder: React.FC<z.infer<typeof videoSchema>> = ({
    segments,
    audio,
    settings,
}) => {
    ...
    const showingPhotoIndex =
        activePhotoIndex === -1
            ? frame > introDuration + bioIntroDuration
        ...
        <>
            ...
            <Sequence durationInFrames={introDuration}>
                <AbsoluteFill
                    style={{
                        background:
                            "linear-gradient(to right, #ea3374, #e45d4c)",
                        fontFamily,
                    }}
                >
                    <TinderLogo pulse />
            ...
            <Sequence from={introDuration - 15}>
                <AbsoluteFill className="bg-white" style={{ fontFamily }}>
                    <section className="flex flex-row justify-between items-center h-36 p-8 z-20 bg-white relative">
                        <Img
                            src={staticFile("assets/tinder-logo.png")}
                            className="h-30"
                        />
                    ...
                    <div
                        className={clsx(
                            "absolute inset-0 z-50 top-96 w-fit",
                            settings.swipe === "left" ? "left-1/2" : "left-20",
                        )}
                        style={{
                    ...
                    <section className="relative w-full min-h-[85%] overflow-hidden bg-black">
                        {photoSegments.map((photo, index) => {
                            ...
                            const timeSinceEnd =
                                frame -
                                (photo.startFrame + photo.durationInFrames);

                            const isFirstPhoto = index === 0;
                            const isLastPhoto =
                                index === photoSegments.length - 1;
                            ...
                            const entranceOffset = isFirstPhoto
                                ? 0
                                : interpolate(
                                      slideInProgress,
                                      [0, 1],
                    ...
                    <section className="absolute bottom-[144px] w-full p-8 z-30 text-white">
                        <h1 className="text-8xl flex items-baseline gap-4">
                            ...
                            <span className="text-6xl font-normal opacity-90">
                        ...
                        <p className="text-4xl leading-snug flex flex-wrap gap-x-3">
                            {settings?.bio_roast.map(
                                (roastItem: any, i: number) => {
                                    const matchedSegment = timedSegments.find(
                                        (s) =>
                                            sanitizeText(s.text) ===
                                            sanitizeText(roastItem.narration),
                                    );
                                    let itemWeight = 0;
                                    ...
                                    if (matchedSegment) {
                                        ...
                                        const exit = spring({
                                            frame:
                                                frame -
                                                (matchedSegment.startFrame +
                                                    matchedSegment.durationInFrames),
                                            fps,
                                            config: springConfig,
                                        });
                                    ...
                                    const globalRoastWeight =
                                        isAnyBioSegmentActive ? 1 : 0;
                    ...
                    <section className="flex flex-row justify-around items-center h-full p-8 z-20 bg-white relative shadow-[0_-10px_40px_rgba(0,0,0,0.1)]">
                        <Img
                            src={staticFile("assets/tinder-icon.png")}
                            className="size-20"
                        />
