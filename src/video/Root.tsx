...
async function calculateMetadata({
    ...
    props: z.infer<typeof videoSchema>;
}) {
    const duration = props.audio.reduce(
        (acc, curr) => acc + (curr.duration || 0),
        0,
    );

    if (props.background.video?.src && !props.background.video.initTime) {
        const video = await fetch(staticFile(props.background.video.src)).then(
            (res) => res.blob(),
        );
        ...
        const randomOffset =
            random(props.background.seed) * videoNeedsToBeginAtMax;
    ...
    const segments =
        props.audio.length > 1
            ? props.segments.map((segment, segmentIndex) => {
                  const tokens = sanitizeText(segment.text).split(/\s+/);

                  const duration = props.audio[segmentIndex]?.duration ?? 0;
                  const segmentAlignment =
                      props.audio[segmentIndex]?.alignment ?? [];
            ...
            : props.segments.map((segment) => {
                  ...
                  const duration =
                      (props.audio[0].alignment?.[pos + tokens.length - 1]
                          ?.end || 0) -
                      (props.audio[0].alignment?.[pos]?.start || 0);
                  const segmentAlignment =
                      props.audio[0]?.alignment?.slice(
                          pos,
                          pos + tokens.length,
                      ) || [];
