...
export const DebateLandscape: React.FC<z.infer<typeof videoSchema>> = ({
    segments,
    background,
    audio,
    settings,
    ...
    return (
        <AbsoluteFill style={{ backgroundColor: background.color, fontFamily }}>
            {background.video?.src ? (
                ...
                <Img
                    src={staticFile(background.gif.src)}
                    className="absolute w-full h-full object-cover"
                />
            ) : null}
            ...
            {segments.map((segment, index, segments) => {
                ...
                const mediaType =
                    segment.mediaSrc &&
                    getMimetypeFromFilename(segment.mediaSrc).type;
                                ...
                                return (
                                    ...
                                    >
                                        <AbsoluteFill className="max-w-[85%] max-h-1/3 bottom-0 right-0 left-[unset] top-[unset] px-16 py-8">
                                            <Text
                                                alignedWords={sentence.words}
                                                highlightColor={
                                                    speakerMap[speaker].color
                                                }
                                            />
                                        </AbsoluteFill>
                                    </Sequence>
