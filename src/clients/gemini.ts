...
export class GeminiClient implements ImageGeneratorClient, TTSClient, LLMClient {
    ...
    async generateThumbnail({
        ...
        const imageResult = await genAI.models.generateContent({
            model: 'gemini-3.1-flash-image',
            contents: [
                { text: `You are a thumbnail generator AI. Your task is to create a thumbnail for a ${orientation === 'Portrait' ? 'TikTok' : 'Youtube'} video based on the provided details. Always generate a thumbnail suitable for ${orientation === 'Portrait' ? 'TikTok' : 'Youtube'}. The thumbnail should be visually appealing and relevant to the content of the video. The text should be concise and engaging, ideally no more than 5 words in ${thumbnailTextLanguage}. The thumbnail should include the person acting some action related to the video topic.` },
